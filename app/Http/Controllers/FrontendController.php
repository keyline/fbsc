...
class FrontendController extends Controller
{
    ...
    public function add_member_business(Request $request, $memberId)
    {
        $member = Member::where('uuid',$memberId)->get();
        return view('frontend.user.add-business',['member' => $member]);
    }
   ...
   public function uploadCroppedImage(Request $request)
    {
        try {
            ...
            $filePath = 'uploads/member_profile_pics/' . $fileName;
    
            // Move the uploaded file to the public folder
            $file->move(public_path('uploads/member_profile_pics'), $fileName);
    
            return response()->json([
                'success' => true,
                'path' => $filePath, // Return the full URL of the saved image
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
    ...
    public function uploadCroppedProfile(Request $request)
    {
        try {
            ...
            $file = $request->file('profile_pic');
            $fileName = 'profile_pic_' . $userId . '_' . time() . '.' . $file->getClientOriginalExtension();
            $filePath = 'uploads/profile_pics/' . $fileName;
    ...
    public function checkUsernameAvailability(Request $request) {
        ...
        // Check if the username is available in the 'members' table
        $isAvailableInMembers = Member::where('username', $username)->count() === 0;
    
        // Check if the username is available in the 'users' table
        $isAvailableInUsers = User::where('username', $username)->count() === 0;
    
        // Combine the results from both tables
        $isAvailable = $isAvailableInMembers && $isAvailableInUsers;
    ...
    protected function create_member(Request $request)
    {
        ...
        $this->validate($request, [
            ...
            'blood_group' => 'nullable|string|max:191',
        ]);

        // $input1 = implode(',', $request->committee);
        ...
        try {
            ...
            if ($request->cropped_profile_pic) {

                // Generate a unique filename for the uploaded image
                // $fileName = 'profile_pic_' . time() . '.' . $request->file('profile_pic')->getClientOriginalExtension();
                // $paths = 'uploads/member_profile_pics/' . $fileName;
                // $request->file('profile_pic')->move(public_path('uploads/member_profile_pics'), $paths);

                // Storage::putFileAs('profile_pics/' , $request->file('profile_pic'), $fileName);
                $member->profile_pic = $request->cropped_profile_pic;
            }
            // if ($request->hasFile('cover_pic')) {
            //     // Generate a unique filename for the uploaded image
            //     $fileName = 'profile_cover_pic_' . time() . '.' . $request->file('cover_pic')->getClientOriginalExtension();
            //     $paths = 'uploads/member_profile_cover_pics/' . $fileName;
            //     $request->file('cover_pic')->move(public_path('uploads/member_profile_cover_pics'), $paths);
            //     $member->profile_cover_pic = $paths;
            // }
           ...
           // return view('frontend.user.add-business', ['memberId' => $memberId])->with(['msg' => __('Registration Success'), 'type' => 'success']);
            return redirect()->route('add_member_business', ['memberId' => $memberId])->with(['msg' => __('Registration Success. Add Your Business/Profession, afer Submit We will review your submission, and if approved, you will receive an email with further instructions.'), 'type' => 'success']);
       
        } catch (\Exception $e) {
            // Handle the exception, log the error, or print the error message for debugging
            Log::error('Error adding member and user details: ' . $e->getMessage());

            // dd($e->getMessage());
            return redirect()->back()->with(['msg' => __('Somethings Going Wrong! Please Try Again'. $e->getMessage()), 'type' => 'danger']);
        }
    }

    public function store_member_business(Request $request)
    {
        ...
        $business = new Business();
         ...
         $business->member_id = $request->member_id;
        ...
        return response()->json(['message' => 'Business Added successfully']);
