                            ...
                            @if (auth()->check())
                                ...
                                <li class="dropdown">
                                    ...
                                    <div class="dropdown-menu " aria-labelledby="dropdownMenuButton">
                                      <a class="dropdown-item text-dark" href="{{ $route }}">{{ __('Profile') }}</a>
                                      <a class="dropdown-item text-dark" href="{{route('user.business')}}">Business dashboard</a>
                                       @php
                                          $user = Auth::user();
                                      @endphp
                                     @if($user->is_member == 'yes')
                                      <a class="dropdown-item text-dark" href="{{route('user.members')}}">Members</a>
                                      <a class="dropdown-item text-dark" href="{{route('user.offers')}}">Offers</a>
                                      @endif
                                      <a class="dropdown-item text-dark"  href="{{ route('user.logout') }}" onclick="event.preventDefault();
                                        document.getElementById('userlogout-form').submit();">
                                        {{ __('Logout') }}
                                    </a>
                                    <form id="userlogout-form" action="{{ route('user.logout') }}" method="POST"
